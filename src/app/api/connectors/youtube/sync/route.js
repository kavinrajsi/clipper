...
export async function POST() {
  ...
  try {
    ...
    if (videos.length > 0) {
      const { error: videosError } = await supabase.from("youtube_videos").upsert(
        videos.map((video) => ({
          user_id: user.id,
    ...
    if (analytics.length > 0) {
      const { error: statsError } = await supabase.from("youtube_channel_stats_daily").upsert(
        analytics.map((row) => ({
          user_id: user.id,
          date: row.day,
    ...
    if (activities.length > 0) {
      const { error: activitiesError } = await supabase.from("youtube_activities").upsert(
